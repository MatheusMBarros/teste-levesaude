...
- `especialidade_sugerida` restrita a uma lista fechada (as especialidades dos mocks, incluindo "Clínico Geral"). A garantia vem do `enum` da ferramenta do LLM e da validação Zod da saída; "Clínico Geral" para casos vagos é regra do prompt, não fallback no código: especialidade fora da lista é resposta inválida (ADR-010).
...
| D8  | Payload inválido                                                                 | 400 `{ erro: "Payload inválido", mensagem, detalhes: [{ campo, problema }] }` — inclui JSON malformado e body ausente; problema no corpo inteiro usa `campo: "corpo"`                                                                                                                                                                                                                                                                         | Erro acionável pelo cliente                                                                                                                                                                               |
...
| D15 | Estado compartilhado entre `GET /agendas`, `POST /agendamento` e `POST /triagem` | **Uma única função Lambda `schedule`** com os três eventos `http`; o handler despacha por tabela `resource + httpMethod` → controller (sem `if` de negócio). A triagem entrou na mesma função porque `medicos_disponiveis` precisa enxergar as reservas (ADR-009); timeout da função: 20 s                                                                                                                                                    | Lambdas distintas nunca compartilham memória (na AWS; no serverless-offline v13 o modo padrão _worker-threads_ isola cada handler; o serverless-esbuild gera um bundle por função). Ver ADR-008 e ADR-009 |
...
| D19 | Erros gerados pelo API Gateway (rota inexistente, 403 etc.) e CORS               | `GatewayResponses` DEFAULT_4XX/DEFAULT_5XX no formato `{ erro, mensagem }`; `MISSING_AUTHENTICATION_TOKEN` (rota/método inexistente, 403 por padrão na AWS REST) vira 404 `{ erro: "Rota não encontrada", ... }`. O serverless-offline **não emula** `GatewayResponses`: localmente a rota inexistente devolve o 404 próprio dele, fora do formato; header `Access-Control-Allow-Origin` em **todas** as respostas da Lambda, inclusive erros | `cors: true` não adiciona headers em respostas de proxy; formato de erro consistente (D9)                                                                                                                 |
...
| D25 | Resposta da triagem em emergência                                                | Com `urgencia: "emergencia"`, a `justificativa` começa com o texto fixo "Procure imediatamente um pronto-socorro ou ligue 192 (SAMU).", seguido da justificativa do modelo. `aviso` e `medicos_disponiveis` continuam na resposta                                                                                                                                                                                                             | A orientação de emergência não pode depender do texto gerado pelo modelo; manter os médicos não atrapalha e serve ao acompanhamento depois do atendimento                                                 |
| D26 | `medicos_disponiveis` da triagem                                                 | Médicos da especialidade sugerida com pelo menos um horário livre, na ordem do seed (D23). `proximo_horario` = o horário livre **mais cedo** do médico, mesmo com a agenda fora de ordem no seed. Médico sem horário livre é omitido; nenhum médico → lista vazia. A agenda é lida depois da resposta do modelo                                                                                                                               | O paciente precisa de um horário agendável agora; lista vazia é resposta válida (a sugestão continua útil)                                                                                                |
| D27 | Relato do paciente no prompt (prompt injection)                                  | O relato vai só na mensagem do usuário, entre `<sintomas>` e `</sintomas>`, escapado: `&` → `&amp;` primeiro, depois `<` → `&lt;` e `>` → `&gt;`; caracteres de controle C0 removidos, exceto `\n` e `\t`. O system prompt manda tratar o conteúdo como dado. Sintomas nunca vão para log                                                                                                                                                     | O paciente não consegue fechar a tag nem injetar seções; risco residual documentado na ADR-011                                                                                                            |
| D28 | Falhas e retentativas da triagem                                                 | Retentativa própria (SDK com `maxRetries: 0`), no máximo 3 chamadas por triagem, 5 s por tentativa. Timeout → 504 sem retentar. 429/5xx/conexão → backoff com jitter; esgotou ou `retry-after` > 1 s → 503. Outro erro da API (401, 404...) → 503 sem retentar. Saída inválida → 1 retentativa, depois 502. Pior caso de 17 s dentro dos 20 s da função                                                                                       | Orçamento de tempo previsível e sempre com resposta no formato D9. Ver ADR-010                                                                                                                            |
...
| Urgência (`baixa`/`media`/`alta`/`emergencia`) | `Urgency` (value object, `URGENCY_LEVELS`)                                                                                                                                         |
...
| Triagem (adapters)                             | `AnthropicTriageModel`, `FakeTriageModel`, `UnavailableTriageModel` (sem chave, 503), `triage.prompt.v1.ts`                                                                        |
| Cliente do SDK visto pelo adapter              | `AnthropicMessagesClient` (interface estreita, stub nos testes)                                                                                                                    |
| Configuração inválida no boot                  | `InvalidConfigError` (`src/main/env.schema.ts`; ex.: `TRIAGE_PROVIDER` desconhecido derruba o init da função — ADR-009). Não é `DomainError`: é falha de deploy, não de requisição |
