...
| D4  | Médico inexistente                                                 | 404 `{ erro: "Médico não encontrado", mensagem }`                                                                                                                                                              | Recurso referenciado não existe                                                                                                                                                                   |
| D5  | Horário que nunca esteve na agenda do médico                       | 422 `{ erro: "Horário não ofertado", mensagem }`                                                                                                                                                               | Diferente de "ocupado" (409). Registrar ADR; alternativa aceitável: 409                                                                                                                           |
...
| D16 | Idioma do código                                                   | Identificadores **em inglês** (classes, funções, variáveis, arquivos, env vars). Português só no contrato HTTP (chaves snake_case do enunciado), mensagens ao usuário e descrições de testes. Glossário abaixo | Padrão internacional (`docs/regras/api.md`)                                                                                                                                                       |
